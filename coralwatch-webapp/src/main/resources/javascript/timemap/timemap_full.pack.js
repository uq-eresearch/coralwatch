...
function TimeMap(d, g, l) {
    ...
    var j = l.mapCenter || new GLatLng(0, 0),h = l.mapZoom || 0,b = l.mapType || G_PHYSICAL_MAP,k = l.mapTypes || [G_NORMAL_MAP,G_SATELLITE_MAP,G_PHYSICAL_MAP],c = ("showMapTypeCtrl" in l) ? l.showMapTypeCtrl : true,f = ("showMapCtrl" in l) ? l.showMapCtrl : true;
    this.opts.syncBands = ("syncBands" in l) ? l.syncBands : true;
    this.opts.mapFilter = l.mapFilter || TimeMap.filters.hidePastFuture;
    this.opts.centerOnItems = ("centerMapOnItems" in l) ? l.centerMapOnItems : true;
...
TimeMap.init = function(c) {
    ...
    if (!c.bandInfo && !c.bands) {
        var i = c.bandIntervals || c.options.bandIntervals || [DT.WEEK,DT.MONTH];
        if (typeof(i) == "string") {
    ...
    var l = new TimeMap(document.getElementById(c.timelineId), document.getElementById(c.mapId), c.options);
    var g = [],k,b,e,a;
...
TimeMap.loadManager = new function() {
    ...
    this.complete = function() {
        ...
        if (c) {
            ...
            if (b && e.getCount() > 0) {
                switch (b) {case"now":break;case"earliest":f = e.getEarliestDate();break;case"latest":f = e.getLatestDate();break;default:if (typeof(b) == "string") {
...
TimeMap.intervals = {sec:[DT.SECOND,DT.MINUTE],min:[DT.MINUTE,DT.HOUR],hr:[DT.HOUR,DT.DAY],day:[DT.DAY,DT.WEEK],wk:[DT.WEEK,DT.MONTH],mon:[DT.MONTH,DT.YEAR],yr:[DT.YEAR,DT.DECADE],dec:[DT.DECADE,DT.CENTURY]};
TimeMap.mapTypes = {normal:G_NORMAL_MAP,satellite:G_SATELLITE_MAP,hybrid:G_HYBRID_MAP,physical:G_PHYSICAL_MAP,moon:G_MOON_VISIBLE_MAP,sky:G_SKY_VISIBLE_MAP};
...
TimeMap.prototype.addFilterChain = function(c, b, a) {
    this.filters[c] = {chain:[],on:b,off:a}
};
...
TimeMapDataset.dateParsers = {hybrid:TimeMapDataset.hybridParser,iso8601:DT.parseIso8601DateTime,gregorian:TimeMapDataset.gregorianParser};
...
TimeMapDataset.prototype.loadItem = function(B, r) {
    ...
    var j = B.options || {},g = this.timemap,c = this.opts.theme,A = j.theme || c;
    ...
    if (m !== null) {
        ...
        if (TimeMap.util.TimelineVersion() == "1.2") {
            w = new o(m, k, null, null, e, C, null, null, null, d, A.eventColor, A.eventTextColor)
        } else {
            var l = A.eventTextColor;
            if (!l) {
                l = (A.classicTape && !e) ? "#FFFFFF" : "#000000"
            }
            w = new o({start:m,end:k,instant:e,text:C,icon:d,color:A.eventColor,textColor:l})
        }
    }
    ...
    var t = function(E) {
        ...
        if ("point" in E) {
            ...
            if ("polyline" in E || "polygon" in E) {
                ...
                if ("polyline" in E) {
                    D = new GPolyline(J, A.lineColor, A.lineWeight, A.lineOpacity);
                    F = "polyline";
                    H = D.getVertex(Math.floor(D.getVertexCount() / 2))
                } else {
                    D = new GPolygon(J, A.polygonLineColor, A.polygonLineWeight, A.polygonLineOpacity, A.fillColor, A.fillOpacity);
                    F = "polygon";
                    H = D.getBounds().getCenter()
                }
            } else {
                if ("overlay" in E) {
                    var I = new GLatLng(parseFloat(E.overlay.south), parseFloat(E.overlay.west));
                    var p = new GLatLng(parseFloat(E.overlay.north), parseFloat(E.overlay.east));
                    if (g.opts.centerOnItems) {
                        n.extend(I);
                        n.extend(p)
                    }
        ...
        return{placemark:D,type:F,point:H}
    };
    ...
    j.theme = A;
    if (j.infoPoint) {
        j.infoPoint = new GLatLng(parseFloat(j.infoPoint.lat), parseFloat(j.infoPoint.lon))
    } else {
        j.infoPoint = v
    }
...
function TimeMapItem(d, e, g, c) {
    ...
    this.closeInfoWindow = c.closeInfoWindow || a.closeInfoWindow || f.closeInfoWindow || TimeMapItem.closeInfoWindowBasic
}
...
TimeMapItem.prototype.showEvent = function() {
    if (this.event) {
        if (this.eventVisible === false) {
            this.dataset.timemap.timeline.getBand(0).getEventSource()._events._events.add(this.event)
        }
        this.eventVisible = true
    }
};
TimeMapItem.prototype.hideEvent = function() {
    if (this.event) {
        if (this.eventVisible == true) {
            this.dataset.timemap.timeline.getBand(0).getEventSource()._events._events.remove(this.event)
        }
...
TimeMapItem.openInfoWindowBasic = function() {
    ...
    if (a === "") {
        ...
        if (this.opts.description !== "") {
            a += '<div class="infodescription">' + this.opts.description + "</div>"
        }
    }
...
TimeMap.util.formatDate = function(h, g) {
    ...
    if (h) {
        ...
        i += f + "-" + a(b + 1) + "-" + a(j);
        if (g > 1) {
            var c = h.getUTCHours(),e = h.getUTCMinutes(),k = h.getUTCSeconds();
            i += "T" + a(c) + ":" + a(e);
            if (g > 2) {
                i += a(k)
            }
...
TimeMap.prototype.toJSON = function() {
    var a = {options:this.makeOptionData,datasets:this.datasets};
...
TimeMapDataset.prototype.toJSON = function() {
    var a = {title:this.getTitle(),theme:TimeMap.util.revHash(TimeMapDataset.themes, this.opts.theme),data:{type:"basic",value:this.getItems()}};
    a = this.addExportData(a);
    return a
};
...
TimeMapItem.prototype.toJSON = function() {
    var d = {title:this.getTitle(),options:{description:this.opts.description}};
    if (this.event) {
    ...
    if (this.placemark) {
        ...
        var c = function(h, g, i) {
            ...
            switch (h) {case"marker":i.point = a.makePoint(g.getLatLng());break;case"polyline":case"polygon":var f = [];for (var e = 0; e < g.getVertexCount(); e++) {
...
TimeMapItem.prototype.createEvent = function(b, d) {
    var a = (d === undefined);
    var f = this.getTitle();
    var c = new Timeline.DefaultEventSource.Event(b, d, null, null, a, f, null, null, null, this.opts.theme.eventIcon, this.opts.theme.eventColor, null);
    c.item = this;
    this.event = c;
...
TimeMapItem.prototype.changeTheme = function(c) {
    ...
    if (this.placemark) {
        var b = function(d, e, f) {
            ...
            switch (e) {case"marker":d.setImage(f.icon.image);break;case"polygon":d.setFillStyle({color:c.fillColor,opacity:c.fillOpacity});case"polyline":d.setStrokeStyle({color:c.lineColor,weight:c.lineWeight,opacity:c.lineOpacity});break}
        };
        if (this.getType() == "array") {
            for (var a = 0; a < this.placemark.length; a++) {
...
TimeMapItem.prototype.getNext = function(a) {
    ...
    var b = d.getEventIterator(this.event.getStart(), new Date(d.getLatestDate().getTime() + 1));
    var c = null;
    while (c === null) {
        if (b.hasNext()) {
            c = b.next().item;
...
TimeMap.loaders.flickr = function(b) {
    ...
    a.transform = function(d) {
        var c = {title:d.title,start:d.date_taken,point:{lat:d.latitude,lon:d.longitude},options:{description:d.description.replace(/&gt;/g, ">").replace(/&lt;/g, "<").replace(/&quot;/g, '"')}};
...
TimeMap.loaders.gss = function(b) {
    var a = new TimeMap.loaders.jsonp(b);
    if (!a.url) {
        a.url = "http://spreadsheets.google.com/feeds/list/" + b.key + "/1/public/values?alt=json-in-script&callback="
    }
    a.map = b.map;
    a.preload = function(c) {
        return c.feed["entry"]
    ...
    a.transform = function(f) {
        ...
        var e = {title:c("title"),start:c("start"),point:{lat:c("lat"),lon:c("lon")},options:{description:c("description")}};
        if (b.transformFunction) {
...
TimeMap.loaders.gss.map = {title:"title",description:"description",start:"start",end:"end",lat:"lat",lon:"lon"};
...
TimeMap.loaders.metaweb = function(b) {
    ...
    var d = b.query || {};
    var c = encodeURIComponent(JSON.stringify({qname:{query:d}}));
