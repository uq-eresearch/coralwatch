...
(function(F) {
    ...
    var G = {};
    var L = 2.399963;
    F.tagcloud = {id:"TagCloud",version:"0.5.0",defaults:{height:null,type:"cloud",sizemax:20,sizemin:10,colormax:"00F",colormin:"B4D2FF",seed:null,power:0.5}};
    F.fn.extend({tagcloud:function(M) {
        C = F.extend({}, F.tagcloud.defaults, M);
        if (C.seed === null) {
            C.seed = Math.ceil(Math.random() * 45309714203)
        }
        switch (C.type) {case"sphere":case"cloud":A = {position:"relative"};G = {position:"absolute",display:"block"};break;case"list":A = {height:"auto"};G = {position:"static",display:"inline"};break}
        ...
        return this.each(function(f, a) {
            ...
            q.css({width:Z,height:l,listStyle:"none",margin:0,padding:0});
            q.css(A);
            var e = -2147483647;
            var r = 2147483648;
            var T = -1;
            ...
            for (var d = 0; d < S; d++) {
                ...
                p.css({fontSize:m,position:"absolute",color:"#" + N,margin:0,padding:0}).children().css({color:"#" + N});
                var Y = p.width();
                var h = p.height();
                var Q = {};
                if (C.type != "list") {
    ...
    F.fn.TagCloud = F.fn.Tagcloud = F.fn.tagcloud
})(jQuery);